...
public class ALU {
	...
	public void doOperation(Bit[] operation) {
		...
		case AND -> result = op1.and(op2);
		case OR -> result = op1.or(op2);
		case XOR -> result = op1.xor(op2);
		case NOT -> result = op1.not();
		case LEFT_SHIFT -> {
	...
	public static Bit notEquals(Word op1, Word op2) {
		...
		return result.getBit(0).or(result.getBit(1)).or(result.getBit(2)).or(result.getBit(3)).or(result.getBit(4))
				.or(result.getBit(5)).or(result.getBit(6)).or(result.getBit(7)).or(result.getBit(8))
				.or(result.getBit(9)).or(result.getBit(10)).or(result.getBit(11)).or(result.getBit(12))
				.or(result.getBit(13)).or(result.getBit(14)).or(result.getBit(15)).or(result.getBit(16))
				.or(result.getBit(17)).or(result.getBit(18)).or(result.getBit(19)).or(result.getBit(20))
				.or(result.getBit(21)).or(result.getBit(22)).or(result.getBit(23)).or(result.getBit(24))
				.or(result.getBit(25)).or(result.getBit(26)).or(result.getBit(27)).or(result.getBit(28))
				.or(result.getBit(29)).or(result.getBit(30)).or(result.getBit(31));
	}

	public Bit doBooleanOperation(Bit[] operation) {
		// TODO: should bops be in alu?
		return switch (getOp(operation)) {
		case EQ -> notEquals(op1, op2).not();
		case NEQ -> notEquals(op1, op2);
		case LT -> sub(op1, op2).getBit(31);
		case GE -> sub(op1, op2).getBit(31).not();
		case GT -> sub(op2, op1).getBit(31);
		case LE -> sub(op2, op1).getBit(31).not();
	...
	private static Tuple<Bit, Triple<Bit, Bit, Bit>> add4(Bit w, Bit x, Bit y, Bit z, Triple<Bit, Bit, Bit> cin) {
		...
		var couts = getNewCarries.apply(x).andThen(getNewCarries.apply(y)).andThen(getNewCarries.apply(z))
				...
				.andThen(getNewCarries.apply(cin3))
				.apply(new Tuple<>(w, new Triple<Bit, Bit, Bit>(new Bit(false), new Bit(false), new Bit(false)))).snd();
		return new Tuple<>(s2, couts);
	}

	protected static Word add4(Word a, Word b, Word c, Word d) {
		var bits = IntStream.range(0, 32).boxed().reduce(
				new Tuple<>(new Bit[32], new Triple<>(new Bit(false), new Bit(false), new Bit(false))), (tuple, i) -> {
	...
	public static Word mul(Word a, Word b) {
		...
		return add(
				add4(add4(indexToWord.apply(0), indexToWord.apply(1), indexToWord.apply(2), indexToWord.apply(3)),
						add4(indexToWord.apply(4), indexToWord.apply(5), indexToWord.apply(6), indexToWord.apply(7)),
						add4(indexToWord.apply(8), indexToWord.apply(9), indexToWord.apply(10), indexToWord.apply(11)),
						add4(indexToWord.apply(12), indexToWord.apply(13), indexToWord.apply(14),
								indexToWord.apply(15))),
				add4(add4(indexToWord.apply(16), indexToWord.apply(17), indexToWord.apply(18), indexToWord.apply(19)),
