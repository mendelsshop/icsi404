...
public class Parser {
	...
	private final HashMap<TokenType, Supplier<Result<Instruction, ParseError>>> parsers = new HashMap<>() {
		{
			/*
			 * TODO: should we have to differnt jump instruction b/c their both essentially
			 * NoR TODO: for 3r push an branch they are the same as their repsective 2rs,
			 * becuase they dont use their rd (make more instructin?)
			 */
			...
			put(TokenType.BRANCH, parseInstruction(InstructionType.BRANCH, Optional.of(Parser::isBooleanOperator),
					Optional.empty(), Optional.empty(), Optional.of(true), Optional.of(true)));
			put(TokenType.CALLIF, parseInstruction(InstructionType.CALL, Optional.of(Parser::isBooleanOperator),
					Optional.empty(), Optional.empty(), Optional.of(true), Optional.of(true)));
			put(TokenType.JUMP, parseInstruction(InstructionType.BRANCH, Optional.empty(), Optional.of(true),
					Optional.of(true), Optional.empty(), Optional.empty()));
			put(TokenType.CALL, parseInstruction(InstructionType.CALL, Optional.empty(), Optional.of(true),
					Optional.of(true), Optional.empty(), Optional.empty()));
			...
			put(TokenType.COPY,
					() -> parseRegister()
							.map(r -> Instruction.buildWithType(InstructionType.MATH).registers(new DestOnly(r)))
							.flatMap(instruction -> parseNumber().map(n -> instruction.immediate(n))));
			put(TokenType.POP, () -> parseRegister()
					.map(r -> Instruction.buildWithType(InstructionType.POP).registers(new DestOnly(r))));
			...
			put(TokenType.INTERRUPT,
					() -> parseNumber().map(n -> Instruction.buildWithType(InstructionType.POP).immediate(n)));
		}
	};
	...
	public Supplier<Result<Instruction, ParseError>> parseInstruction(InstructionType type,
		...
		Function<Optional<Boolean>, Function<RegisterFormat, Function<Instruction, Result<Instruction, ParseError>>>> validateRegister = (
				hasImmediate) -> registers -> instruction -> hasImmediate
						...
						.apply(instruction)
						// set registers
						.map(setInstructionRegisters.apply(registers));
		...
		((Function<RegisterFormat, Result<Instruction, ParseError>>) registers -> validateRegister
				.apply(registers instanceof NoR ? NoR
						: registers instanceof DestOnly ? DestOnly : registers instanceof TwoR ? TwoR : ThreeR)
				.apply(registers).apply(instruction))
				// parse the regiisters, this may seem a bit counterintutive, but the casted
				// function above where we match on the regsiter format is beind applied to the
				// result of this
		...
		return () -> operator.<Function<Instruction, Result<Instruction, ParseError>>>map(
				...
				resultToOptional(() -> Remove(), new ParseError("could not get function"))
						// make sure its a valid token for this type of function
						.filterOrErr(operatorValidator, new ParseError("expected function"))
	...
	public RegisterFormat parseRegisters() {
		return parseRegister().<RegisterFormat>map(r1 -> parseRegister().map(r2 -> parseRegister()
				...
				.<RegisterFormat>map(r3 -> new ThreeR(r1, r2, r3))
	...
	public Result<Register, ParseError> parseRegister() {
		return resultToOptional(() -> MatchAndRemove(TokenType.REGISTER), new ParseError("could not parse register"))
				.map(r -> new Register(r.getValue().get()));
	}

	public Result<Integer, ParseError> parseNumber() {
		return resultToOptional(() -> MatchAndRemove(TokenType.VALUE), new ParseError("could not parse number"))
		...
		return resultToOptional(() -> Remove(), new ParseError("could not find new start token"))
				...
				.map(token -> parsers.getOrDefault(token.getType(),
						...
						() -> Err.<Instruction, ParseError>err(
								new ParseError("could not find parser for that keyword " + token))))
				// run the parser
