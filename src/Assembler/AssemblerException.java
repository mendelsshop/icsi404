...
public class AssemblerException extends Exception {
	...
	Integer position = 0;
	String message = "";
	private AssemblerException.ExceptionType exception;

	public AssemblerException(Integer lineNumber, Integer position, String message, ExceptionType exception) {
	...
	public void DisplayError(String sourceFile, String sourceFileName) {
		...
		sourceFile.lines().skip(lineNumber - 1).findFirst().ifPresent(line -> {
			...
			System.err.println("\t" + lineNumber.toString().chars().mapToObj(c -> " ").collect(Collectors.joining())
